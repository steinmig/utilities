...
class QuaternionFit {
 public:
   ...
   *
   * @param Rref The atom positions of the reference geometry.
   * @param Rfit The atom positions of the geometry to be fitted.
   * @param elements The elements for the given atoms coordinates, these will be used as weights.
   * @param improperRotationIsAllowed If true allows the algorithm to invert the geometry to generate a better fit.
   */
  template<typename DerivedA, typename DerivedB>
  QuaternionFit(const Eigen::DenseBase<DerivedA>& refMat, const Eigen::DenseBase<DerivedB>& fitMat,
                const ElementTypeCollection& elements, bool improperRotationIsAllowed = false)
    : QuaternionFit(refMat, fitMat, makeWeightsVector(elements), improperRotationIsAllowed) {
  }
  /**
