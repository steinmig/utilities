...
class Generator {
 public:
  Generator(Eigen::MatrixXd dataPoints, int splineDegree);
  Generator(Eigen::MatrixXd dataPoints, int numberOfControlPoints, int splineDegree);
  virtual ~Generator() = default;

  BSpline generateBSpline();

 protected:
  Eigen::MatrixXd dataPoints_;
  int p_, dim_, m_, n_;
